...
function AccountsOverview() {
  const { users, isLoading, error, fetchUsers, deleteUser } = useAdminStore();
  ...
  const owners = list.filter((a) => a.role === 'owner').length;
  const influencers = list.filter((a) => a.role === 'influencer').length;
                  ...
                  <tr key={acc.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Link to={`/dashboard/admin/users/${acc.id}`}>
