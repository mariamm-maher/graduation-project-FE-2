        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-xl sm:text-2xl font-bold text-white mb-1 truncate">{workspace.influencer}</h1>
              <p className="text-sm sm:text-base text-gray-400 truncate">{workspace.campaign}</p>
              <div className="flex items-center gap-2 sm:gap-3 mt-2 flex-wrap">
                <span className={`px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm font-semibold ${getStatusColor(workspace.status)}`}>
                  {workspace.status.toUpperCase()}
                </span>
                <span className="text-xs sm:text-sm text-gray-400">{workspace.progress}% Complete</span>
              </div>
            </div>
          </div>

          <button className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded-xl transition-all text-sm sm:text-base w-full sm:w-auto justify-center">
      ...
      <div className="flex gap-1 sm:gap-2 border-b border-white/10 overflow-x-auto">
        {['brief', 'timeline', 'deliverables', 'payment'].map((tab) => (
          <button
            ...
            className={`px-3 sm:px-6 py-2 sm:py-3 font-semibold capitalize transition-all whitespace-nowrap text-xs sm:text-base ${
              activeTab === tab
              ...
              <div key={deliverable.id} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                  <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-white/5 flex items-center justify-center text-gray-400 shrink-0">
                      {getTypeIcon(deliverable.type)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-white mb-1 text-sm sm:text-base">{deliverable.name}</h4>
                      {deliverable.file && (
                        <p className="text-xs sm:text-sm text-gray-400 mb-2 truncate">{deliverable.file}</p>
                      )}
                  ...
                  <span className={`px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm font-semibold whitespace-nowrap ${getStatusColor(deliverable.status)}`}>
                ...
                {deliverable.file && (
                  <div className="flex flex-wrap gap-2 sm:gap-3 mt-4">
                    <button className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-xs sm:text-sm text-gray-300 transition-all flex-1 sm:flex-initial">
                      <Download className="w-3 h-3 sm:w-4 sm:h-4" />
                      Download
                    </button>
                    {deliverable.status === 'submitted' && (
                      <>
                        <button className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-green-500/20 hover:bg-green-500/30 rounded-lg text-xs sm:text-sm text-green-400 transition-all flex-1 sm:flex-initial">
                          <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4" />
                          Approve
                        </button>
                        <button className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-yellow-500/20 hover:bg-yellow-500/30 rounded-lg text-xs sm:text-sm text-yellow-400 transition-all flex-1 sm:flex-initial">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6">
                ...
                <p className="text-2xl sm:text-3xl font-bold text-white">${workspace.payment.total.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-4 sm:p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-xs sm:text-sm text-gray-400">Paid</span>
                </div>
                <p className="text-2xl sm:text-3xl font-bold text-green-400">${workspace.payment.paid.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-4 sm:p-6">
                ...
                <p className="text-2xl sm:text-3xl font-bold text-yellow-400">${workspace.payment.pending.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6">
              ...
              <div className="space-y-4">
                {workspace.payment.milestones.map((milestone, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 p-4 bg-white/5 rounded-lg">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-white mb-1 text-sm sm:text-base">{milestone.name}</h4>
                      <p className="text-xs sm:text-sm text-gray-400">Due: {milestone.date}</p>
                    </div>
                    <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
                      <p className="text-lg sm:text-xl font-bold text-white">${milestone.amount.toLocaleString()}</p>
                      <span className={`px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm font-semibold whitespace-nowrap ${getStatusColor(milestone.status)}`}>
