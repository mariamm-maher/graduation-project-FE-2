...
function CollaborationWorkspace() {
  ...
  const collab = influencerCollaborations.find(
    (c) => String(c._id || c.id) === String(id)
  );

  if (influencerCollaborationsLoading) {
    return (
  ...
  const completedTasks = tasks.filter(t => String(t.status || '').toLowerCase() === 'approved').length;
  const taskProgress   = tasks.length > 0 ? Math.round((completedTasks / tasks.length) * 100) : 0;

  const fmt = (d) => d ? new Date(d).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : 'Not set';
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-sm font-semibold text-white flex items-center gap-1">
                <Calendar className="w-4 h-4 text-gray-400" />{fmt(startDate)}
              </p>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <p className="text-xs text-gray-400 mb-1">End Date</p>
              <p className="text-sm font-semibold text-white flex items-center gap-1">
                <Calendar className="w-4 h-4 text-gray-400" />{fmt(endDate)}
              </p>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-sm font-semibold text-white flex items-center gap-1">
                ...
                {completedTasks} / {tasks.length} approved
              </p>
            </div>
          </div>

          {/* Campaign goal */}
          {collab?.campaign?.campaign_goal && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-white mb-2">Campaign Goal</h3>
              <p className="text-gray-300 text-sm">{collab.campaign.campaign_goal}</p>
            </div>
          )}
          ...
          {tasks.length === 0 ? (
            ...
            <div className="space-y-3">
              {tasks.map((task, idx) => {
                ...
                return (
                  <div key={task.id || idx} className="flex items-center justify-between gap-4 p-4 bg-white/5 border border-white/10 rounded-lg">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${ts === 'approved' ? 'bg-green-500/20 text-green-400' : ts === 'in_progress' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-gray-500/20 text-gray-400'}`}>
                        {ts === 'approved' ? <CheckCircle className="w-4 h-4" /> : <Clock className="w-4 h-4" />}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-white truncate">{task.taskName || task.title || `Task ${idx + 1}`}</p>
                        {task.dueDate && (
                          <p className="text-xs text-gray-400 mt-0.5">Due: {fmt(task.dueDate)}</p>
                        )}
                      </div>
                    </div>
