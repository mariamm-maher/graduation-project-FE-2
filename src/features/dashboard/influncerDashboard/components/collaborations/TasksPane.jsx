...
import { AlertCircle, ArrowRight, LayoutGrid, ListChecks, Loader2, RefreshCw, XCircle } from 'lucide-react';
import {
  closestCorners,
  DndContext,
  DragOverlay,
...
export default function TasksPane() {
  const { influencerCollaborations, getMyInfluencerCollaborations, isInfluencerCollaborationsLoading } =
  ...
  const collabs = useMemo(() =>
    (influencerCollaborations || []).map((c) => ({
  ...
  const handleDragOver = (event) => {
    ...
    setTasks((prev) => prev.map((t) =>
      t.id === active.id
        ? { ...t, status: nextStatus, completedAt: nextStatus === 'completed' ? new Date().toISOString().slice(0, 10) : null }
        : t
    ));
  };
