...
function getStatus(status = '', requiresAction = false) {
  ...
  if (requiresAction || normalized.includes('pending') || normalized.includes('waiting')) return 'waiting';
  return 'default';
}
...
function getIcon(statusKey) {
  if (statusKey === 'completed') return <CheckCircle2 className="w-4 h-4" />;
  if (statusKey === 'waiting') return <MessageSquare className="w-4 h-4" />;
  if (statusKey === 'saved') return <Send className="w-4 h-4" />;
  ...
  return (
    <div>
      ...
      <div className="space-y-3 max-h-[480px] overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-white/5 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:bg-linear-to-b [&::-webkit-scrollbar-thumb]:from-[#C1B6FD] [&::-webkit-scrollbar-thumb]:to-[#745CB4] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:from-[#745CB4] [&::-webkit-scrollbar-thumb]:hover:to-[#C1B6FD]">
        ...
        {feed.map((activity) => {
          const statusKey = getStatus(activity.status, activity.requiresAction);
          const statusUI = statusConfig[statusKey] || statusConfig.default;
          return (
          <div 
            key={activity.id} 
            ...
            <div className="flex items-start gap-3">
              <div className="relative">
                <div className="w-12 h-12 rounded-full bg-linear-to-br from-[#C1B6FD] to-[#745CB4] flex items-center justify-center text-2xl shadow-lg">
                  <span className="text-sm font-bold text-white">{activity.brand?.name?.charAt(0) || 'B'}</span>
                </div>
                ...
                <div className="flex items-start justify-between mb-1">
                  <div>
                    <h3 className="font-semibold text-white text-sm group-hover:text-[#C1B6FD] transition-colors">
                      {activity.campaign?.name || 'Campaign'}
                    </h3>
                    <p className="text-xs text-gray-400">{activity.brand?.name || 'Brand'}</p>
                  </div>
